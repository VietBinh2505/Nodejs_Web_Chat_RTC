function removeReqContactSent() {
    $(".user-remove-request-contact-sent").unbind("click").on("click", () => { // bắt sự kiện khi user click xoa bạn bè
        let targetId = $(".user-remove-request-contact-sent").data("uid"); // lấy id người dùng ở form xóa yêu cầu kp
        $.ajax({
            url: "/contact/remove-req-contact-sent",
            type: "delete",
            data: { uid: targetId },
            success: function(data) {
                if (data.success) {
                    // tìm trong div có thẻ li có id trùng với targetid để ẩn đi
                    $("#find-user").find(`div.user-remove-request-contact-sent[data-uid = ${targetId}]`).hide();
                    $("#find-user").find(`div.user-add-new-contact[data-uid = ${targetId}]`).css("display", "inline-block");
                    deCreaseNumberNotification("noti_contact_counter", 1); //số đếm ở quản lý liên lạc
                    deCreaseNumberNotifyfContact("count-request-contact-sent");
                    $("#request-contact-sent").find(`li[data-uid = ${targetId}]`).remove();
                    socket.emit("remove-req-contact-sent", { contactid: targetId }); // soc két đưa ra 1 khẩu lệnh và data chuyền đi là contactid
                }
            },
        });
    });
};
socket.on("response-remove-req-contact-sent", (user) => { //lắng nghe response-add-new-contact đưa dữ liệu vào biến user 
    $(".noti_content").find(`div[data-uid= ${user.id}]`).remove(); //xóa thông báo đi 
    $("ul.list-notifyCations").find(`li>div[data-uid= ${user.id}]`).parent().remove(); //xóa thông báo ở phần modal
    $("#request-contact-received").find(`li[data-uid = ${user.id}]`).remove();
    deCreaseNumberNotifyfContact("count-request-contact-received");
    deCreaseNumberNotification("noti_contact_counter", 1); //số đếm ở quản lý liên lạc
    deCreaseNumberNotification("noti_counter", 1); // số đếm ở thông báo
});

$(document).ready(()=>{
    removeReqContactSent();
});